...
function OptionsList({ groups, selected, onSelect, mobile }) {
  if (groups.length === 0) {
    ...
    <div key={group.label} className="py-1">
      ...
      {group.types.map((type) => (
        <button
          ...
          className={cn(
            'flex w-full items-center rounded-lg px-3 text-left outline-none transition-colors',
      ...
      <>
        ...
        {open && (
          <div className="fixed inset-0 z-[100] flex flex-col justify-end">
            ...
            {/* Sheet */}
            <div className="relative z-10 flex max-h-[70dvh] flex-col rounded-t-2xl bg-popover shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
              {/* Drag handle */}
              ...
              {/* Search */}
              <SearchInput value={search} onChange={setSearch} inputRef={searchRef} mobile />

              {/* Scrollable list */}
              <div className="flex-1 overflow-y-auto overscroll-contain px-1 pb-[env(safe-area-inset-bottom,8px)]">
                <OptionsList groups={filteredGroups} selected={value} onSelect={handleSelect} mobile />
              </div>
            </div>
          </div>
        )}
