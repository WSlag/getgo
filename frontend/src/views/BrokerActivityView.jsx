  ...
  const renderItemBody = (item) => (
    <>
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`shrink-0 px-2 py-0.5 rounded-full text-xs font-medium capitalize ${statusClass(item.statusBucket || item.status)}`}>
      ...
      <div className="mt-2.5 pt-2.5 border-t border-gray-200 dark:border-gray-700 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
        ...
        {item.amount ? (
          <span>Amount: <span className="text-gray-700 dark:text-gray-300 font-medium">PHP {Number(item.amount).toLocaleString()}</span></span>
        ) : null}
        {item.activityAt && (
