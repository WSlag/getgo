...
const buildCargoListings = (count = 44) =>
  Array.from({ length: count }, (_, idx) => {
    ...
    const profile = cargoProfiles[idx % cargoProfiles.length];
    ...
    return {
      ...
      shipper: profile,
      company: profile,
      shipperId: `guest-shipper-${pad((idx % cargoProfiles.length) + 1)}`,
      userId: `guest-shipper-${pad((idx % cargoProfiles.length) + 1)}`,
      origin: route.origin,
      ...
      description: `Anonymized listing #${idx + 1} for guest preview mode.`,
      category: 'CARGO',
      pickupDate: 'Flexible',
      images: [],
      cargoPhotos: [],
...
const buildTruckListings = (count = 36) =>
  Array.from({ length: count }, (_, idx) => {
    ...
    const trucker = truckProfiles[idx % truckProfiles.length];
    ...
    return {
      ...
      type: 'truck',
      trucker,
      truckerId: `guest-trucker-${pad((idx % truckProfiles.length) + 1)}`,
      userId: `guest-trucker-${pad((idx % truckProfiles.length) + 1)}`,
      ...
      bidCount: idx % 6,
      availableDate: 'Today',
      description: `Anonymized truck availability #${idx + 1} for guest preview mode.`,
      truckPhotos: [],
    };
  });
