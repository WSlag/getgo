...
const initializeFirebaseAdmin = () => {
  ...
  try {
    ...
    // Option 4: Use ADC (Google Cloud runtime) if available
      ...
      const projectId = process.env.FIREBASE_PROJECT_ID || process.env.GCLOUD_PROJECT || 'karga-ph';
      firebaseApp = admin.initializeApp({ projectId });
      console.warn(`Firebase Admin initialized in local mode with projectId=${projectId}`);
    } else {
